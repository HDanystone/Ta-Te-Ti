...
export function turnoIAFacil(tablero, ia) {
  for (var i = 0; i < 2; i++) {
    ...
    for (const combinacion of COMBINACIONES_GANADORAS) {
      ...
      if (tablero[a] === jugadorActual && tablero[b] === jugadorActual
        && tablero[c] === null) return c
      if (tablero[a] === jugadorActual && tablero[c] === jugadorActual
        && tablero[b] === null) return b
      if (tablero[c] === jugadorActual && tablero[b] === jugadorActual
...
export function cuentaCeldasVacias(tablero) {
  let array = []
  tablero.forEach((elemento, indice) => { if (elemento === null) array.push(indice) })
  return array
}
